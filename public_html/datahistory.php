...
if (!function_exists('generate_date_today')) {
	...
	function generate_date_today($Format, $Timestamp, $Language = "en", $TimeText = true )
	{
		...
		while($tblhistory = $result->fetch_assoc()) {
			
			
			?>

				<li onclick="window.location.href='story.php?id=<?=$tblhistory[content_id]?>'"><span style="font-weight: 600;"><? echo $tblhistory["contentName"] ?></span><span style='float:right;font-size:14px;'><? echo generate_date_today("d M Y H:i", $tblhistory['historyDate'], "en", true) ;?></span></li>
			  <?    
			}
